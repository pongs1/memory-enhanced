...
在 `$WORKSPACE/AGENTS.md` 末尾追加：

```markdown
## 记忆系统（memory-enhanced 插件）

你有 6 个记忆工具：

| 工具 | 使用时机 |
|---|---|
| `memory_record` | 记录决策、偏好、洞察、错误（重要性 ≥ 0.5） |
| `memory_explore` | 从某条记忆出发，追溯它的关联记忆 |
| `memory_consolidate` | 会话结束或临近压缩时：衰减+归档+重生成MEMORY.md |
| `memory_status` | 检查记忆系统健康状态 |
| `memory_search` | 语义搜索（内置工具，照常使用） |
| `memory_get` | 精确读取某个记忆文件（内置工具，照常使用） |

**重要规则**：
- 会话开始时：用 `read` 工具读取 `.memory/active/scratchpad.md`
- 会话结束前：读取未提炼事件 → 提炼知识到 `memory/knowledge/` → 调用 `memory_consolidate`
- `MEMORY.md` 由插件自动生成，不要手动编辑
```

...
export function buildSceneReconstructionPrompt(
    ...
    return {
        system: [
            ...
            `- Tentative role must be exactly one of: ${allowedList(ALLOWED_NODE_ROLES)}.`,
            "- Do not use generic labels such as `System`, `Component`, `Module`, `Environment`, `Feature`, or `Actor` as the tentative role.",
            "- Prefer nodes that preserve reusable workflow, decision, constraint, condition, evidence, or checkpoint information over static entity inventory.",
            "- If a noun only names the setting but does not carry future recall value, drop it.",
        ...
        user: [
            ...
            "- example: 网关恢复顺序 | gateway recovery order | workflow | 保留先恢复网关再继续更新的有效工作流",
            "- example: 更新后断连条件 | post-update disconnect condition | condition | 保留只在更新后断连场景下触发的条件",
            "",
...
export function buildRelationScoringPrompt(
    ...
    return {
        system: [
            ...
            "",
            "Rules:",
            `- src role and dst role must each be exactly one of: ${allowedList(ALLOWED_NODE_ROLES)}.`,
            "- Do not use generic role labels such as `System`, `Component`, `Module`, `Environment`, or `Feature` in the table.",
        ...
        user: [
            ...
            "| 节点C / Node C | condition | 节点A / Node A | workflow | condition | valid-when | 0.78 | short evidence |",
            "",
